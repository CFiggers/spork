...
### date.janet
### 
### Utility wrappers around `os/time` and `os/date` for doing
### simple time-intelligent work with dates.
### 
...
(def duration?
  ``
  A `duration` is a struct that contains at least the following keys: 
  ...
  Where larger denominations are not needed (e.g. for durations < 1 day), 
  values should be set to `0`, but they must still be present.
  
  The keys in a well-formed `duration` are a strict subset of those in a 
  `date`, allowing a `date` to be used anywhere a `duration` is expected.
  ``
  (schema/validator
    (and :struct
         (props
...
(defn from-string
  ...
  (put date-peg :main ['* ;format-code])

  (def parsed-date
    (from-pairs (peg/match date-peg date-str)))
...
(defmacro- do-add-or-sub
  [f date years months days hours minutes seconds duration]
  (with-syms [$f $date $years $months $days 
    ...
    ~(let [,$f ,f
       ...
       (when ,$duration
         (assert (duration? ,$duration) (string/format "badly formed duration: %q" ,$duration))
         (let [{:year-day dyear-day
                :hours dhours
                :minutes dminutes
                :seconds dseconds} ,$duration]
           (+= time (* dyear-day 60 60 24))
           (+= time (* dhours 60 60))
           (+= time (* dminutes 60))
           (+= time (* dseconds))))
       ...
       (when ,$duration
         (update ret-date :year |(,$f $ (,$duration :year)))
         (when (and (not (leap-year? (ret-date :year)))
                    (= 1 (ret-date :month))
                    (= 28 (ret-date :month-day)))
           (put ret-date :month-day 27)))
...
(defn add
  ...
  ``
  [date &named years months days hours minutes seconds duration]
  (do-add-or-sub + date years months days hours minutes seconds duration))
...
(defn diff
  ...
  (each period [:year :month :month-day :hours :minutes :seconds]
    (-= (later period) (earlier-date period)))

  (os/date (os/mktime later)))
